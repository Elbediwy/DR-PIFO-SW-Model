...
// this is the main class of the pieo scheduler that will be used later in any usage of the scheduler.
class pieo_scheduler : public bm::ExternType {
 public:
...
// the core code of the pieo scheduler, that enqueue, dequeue or force dequeue packets.
		run_core();
...
void level_controller(std::shared_ptr<packet>& level_packet_ptr, unsigned int level_enq, unsigned int level_deq, std::vector<unsigned int> level_flows_rank, unsigned int level_id)
	{
		...
		{	
			...
			pieo(level_packet_ptr, shaping, level_enq, level_deq, use_updated_rank, \
			level_flows_rank, head_FS, head_FB, \
			time_now, out_deq_pkt_ptr,next_flow_id_empty, flow_id_not_empty_each_level);
...
// the core function of the pieo scheduler, applies enqueue and dequeue operations, to & from each level of the hirarchy, 
...
// This is the pieo queue function which handles 1 flow_scheduler and 1 FIFO at a time, this function is used by each level function
	void pieo(std::shared_ptr<pieo_scheduler::packet> pkt_ptr, unsigned int in_shaping, unsigned int in_enq, unsigned int in_deq, unsigned int in_use_updated_rank, \
		std::vector<unsigned int> in_flows_rank, std::shared_ptr<pieo_scheduler::flow_scheduler>& in_head_FS, std::shared_ptr<pieo_scheduler::fifo_bank>& in_head_FB, \
		unsigned int in_time_now, std::shared_ptr<packet>& out_deq_pkt_ptr, unsigned int& next_flow_id_empty, std::vector<unsigned int> flow_id_not_empty_each_level)
	{
		std::shared_ptr<pieo_scheduler::packet> deq_packet_ptr = NULL;
		std::shared_ptr<pieo_scheduler::flow_scheduler> cur_ptr_FS;
...
// then enqueue this next packet to the flow scheduler enqueue_FS.
		if (in_enq == 1)
		{
			std::shared_ptr<pieo_scheduler::packet> enq_packet_ptr;
			enq_packet_ptr = std::shared_ptr<pieo_scheduler::packet>(std::make_shared<pieo_scheduler::packet>(*pkt_ptr));
		...
		{		
			...
			if (deq_packet_ptr != NULL)
			{
				std::shared_ptr<pieo_scheduler::packet> FB_FS_enq_packet_ptr;
				FB_FS_enq_packet_ptr = std::shared_ptr<pieo_scheduler::packet>(std::make_shared<pieo_scheduler::packet>());
...
// used by pieo function to enqueue inside a certain flow scheduler
	void enqueue_FS(std::shared_ptr<pieo_scheduler::packet> new_packet_ptr, std::shared_ptr<pieo_scheduler::flow_scheduler>& head_FS)
	{
		std::shared_ptr<pieo_scheduler::flow_scheduler> cur_ptr_FS;
		cur_ptr_FS = std::shared_ptr<pieo_scheduler::flow_scheduler>(std::make_shared<pieo_scheduler::flow_scheduler>());
		std::shared_ptr<pieo_scheduler::flow_scheduler> prev_ptr_FS;
		prev_ptr_FS = std::shared_ptr<pieo_scheduler::flow_scheduler>(std::make_shared<pieo_scheduler::flow_scheduler>());
		if (head_FS == NULL)
		{
			head_FS = std::shared_ptr<pieo_scheduler::flow_scheduler>(std::make_shared<pieo_scheduler::flow_scheduler>());
		...
		else
		{
			cur_ptr_FS = head_FS;
			prev_ptr_FS = NULL;
			std::shared_ptr<pieo_scheduler::flow_scheduler> temp_ptr;
			temp_ptr = std::shared_ptr<pieo_scheduler::flow_scheduler>(std::make_shared<pieo_scheduler::flow_scheduler>());
...
// used by pieo function to enqueue inside a certain FIFO bank
	void enqueue_FB(std::shared_ptr<pieo_scheduler::packet> new_packet_ptr, std::shared_ptr<pieo_scheduler::fifo_bank>& head_FB)
	{
		std::shared_ptr<pieo_scheduler::fifo_bank> cur_ptr_FB;
		cur_ptr_FB = std::shared_ptr<pieo_scheduler::fifo_bank>(std::make_shared<pieo_scheduler::fifo_bank>());
		std::shared_ptr<pieo_scheduler::fifo_bank> prev_ptr_FB;
		prev_ptr_FB = std::shared_ptr<pieo_scheduler::fifo_bank>(std::make_shared<pieo_scheduler::fifo_bank>());
		std::shared_ptr<pieo_scheduler::fifo_bank> temp_ptr;
		temp_ptr = std::shared_ptr<pieo_scheduler::fifo_bank>(std::make_shared<pieo_scheduler::fifo_bank>());
		temp_ptr->object = new_packet_ptr;
		temp_ptr->left = NULL;
		temp_ptr->bottom = NULL;
		if (head_FB == NULL)
		{
			head_FB = std::shared_ptr<pieo_scheduler::fifo_bank>(std::make_shared<pieo_scheduler::fifo_bank>());
		...
		else
		{
			...
			{
				std::shared_ptr<pieo_scheduler::fifo_bank> temp_ptr2;
				temp_ptr2 = std::shared_ptr<pieo_scheduler::fifo_bank>(std::make_shared<pieo_scheduler::fifo_bank>());
	...
	void dequeue_FS(std::shared_ptr<pieo_scheduler::packet>& deq_packet_ptr, unsigned int in_shaping, std::shared_ptr<pieo_scheduler::flow_scheduler>& head_FS, unsigned int time_now)
	{
		std::shared_ptr<pieo_scheduler::flow_scheduler> cur_ptr_FS;
		cur_ptr_FS = std::shared_ptr<pieo_scheduler::flow_scheduler>(std::make_shared<pieo_scheduler::flow_scheduler>());
		std::shared_ptr<pieo_scheduler::flow_scheduler> prev_ptr_FS;
...
// used by pieo function to dequeue from a certain FIFO bank
	void dequeue_FB(std::shared_ptr<pieo_scheduler::packet>& deq_packet_ptr, unsigned int flow_id, std::shared_ptr<pieo_scheduler::fifo_bank>& head_FB)
	{
		std::shared_ptr<pieo_scheduler::fifo_bank> cur_ptr_FB;
		cur_ptr_FB = std::shared_ptr<pieo_scheduler::fifo_bank>(std::make_shared<pieo_scheduler::fifo_bank>());
		deq_packet_ptr = NULL;
